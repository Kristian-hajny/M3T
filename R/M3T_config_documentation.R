...
#'  at \doi{10.5281/zenodo.17328718}, "download" to have the package
...
#' Method Variations
#' \itemize{
#'   \item{\bold{Use_ACES} - logical stating whether the ACES CO2 inventory at \doi{10.3334/ORNLDAAC/1943} should be used to disaggregate the stationary combustion and natural gas distribution emissions. Default FALSE.}
#'   \item{\bold{Use_Vulcan} - logical stating whether the Vulcan CO2 inventory at \doi{10.5281/zenodo.15446748} should be used to disaggregate the stationary combustion and natural gas distribution emissions. Default TRUE.}
#'   }
#'
#' Accessing Datasets
#' \itemize{
...
#' Emission Factors and similar
#' \itemize{
#'   \item{\bold{natural_gas_pipeline_emission_factors} - data.frame with columns "Leaks_per_mile" and "Avg_emissions_mol_per_s" and named rows "Bare_Steel", "Cast_Iron", "Coated_steel", and "Plastic". Default uses Weller et al. 2020 at \doi{10.1021/acs.est.0c00437}.}
#'   \item{\bold{natural_gas_res_post_meter_emission_factor} - numeric providing the whole-house residential post-meter emission factor in mol/s. Default uses Fischer et al. 2018 at \doi{10.1021/acs.est.8b03217} who reported 0.5% of residential consumption with 401 Giga cubic feet ~= 7850 giga grams of natural gas consumed per year. This is used as a conversion factor from cubic feet to grams.}
...
#'   \item{\bold{Wastewater_Municipal_Method_Moore} - logical. Use the log-log linear relationship from Moore et al., 2025 at \doi{10.1038/s44221-025-00490-z} and Moore et al., 2023 at \doi{10.1021/acs.est.2c05373} to convert flow rate to emission rate. They measured 96 facilities (~10% of total US flow) and showed scaling their relationship nationally results in more than double the emissions estimated by the GHGI. Default TRUE.}
...
#' Accessing Datasets
#' \itemize{
#'   \item{\bold{Source_wastewater_NLCD} - character stating "M3T" or a filepath pointing to the needed file. \bold{Cannot} be set to "download" as there is no simple way to automatically download these files. The National Land Cover Database (NLCD) at \doi{10.5066/P94UXNTS} is high resolution (30 m) land cover data used to distribute septic emissions. Default "M3T", which is already processed NLCD data, to speed analyses.}
...
#' \doi{10.3334/ORNLDAAC/2346} as published in Bloom et al., 2017 at 
