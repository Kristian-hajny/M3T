...
county_outline_file <- "G:/My Drive/Shepson Group Drive/Kris/Philly Inventory/Raw_data_rewrite/County_Tigerlines/tl_2019_us_county.shp"
...
Output_folder <- "G:/My Drive/Shepson Group Drive/Kris/Philly Inventory/Processed_rewrite/"
...
source("C:/Users/Kristian/Desktop/methane_inventory/src/testing/Inventory_based_disaggregation_oldmethod.R")
...
if(Use_ACES){
  ...
  #convert state/domain scale versions to the proper crs
  all_merge_sf_LCC_state <- st_as_sf(project(vect(all_merge_sf_LCC_state),aces_res))
  all_merge_sf_LCC_domain <- st_as_sf(project(vect(all_merge_sf_LCC_domain),aces_res))
  ...
  cover_all <- raster::extract(aces_res,all_merge_sp_LCC,weights=T,
                               cellnumbers=T,normalizeWeights=F,exact=T)
...
if(Use_Vulcan){
  all_merge_sf_LCC_state <- st_as_sf(project(vect(all_merge_sf_LCC_state),vu_res))
  all_merge_sf_LCC_domain <- st_as_sf(project(vect(all_merge_sf_LCC_domain),vu_res))
  cover_all <- raster::extract(vu_res,all_merge_sp_LCC,weights=T,
                               cellnumbers=T,normalizeWeights=F,exact=T)
...
if(XESMF_check){
  ...
  save_data <- function(input){
    ...
    disaggregation_level <- substring(text = input_name,regexpr("by",input_name),
                                      regexpr("\\[",input_name)-1)
    inventory_name <- strsplit(input_name,"_")[[1]][1]
    input <- raster(project(rast(input),y=rast(d03_rast)))
    #project to a grid with the exact right resolution, extent and origin.
    writeRaster(input,
                paste0(Output_folder,'/testing_oldmethod/',inventory_name,'_',disaggregation_level,'_stat_comb_',total,'_regridded.nc'),
                varname='methane_emissions',
                varunit='mol/km2/s',
                longname=paste0(inventory_name,'_',disaggregation_level,'_stat_comb_',total),
                NAflag=-9999,
