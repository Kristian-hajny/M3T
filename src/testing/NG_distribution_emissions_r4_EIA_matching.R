...
rm(PHMSA_csv,EIA_file,HIFLD_check,HIFLD_check2,HIFLD_file,PHMSA_file,A)
################################################################################
...
ghgrp_nn_ldc_emissions <- fromJSON("https://data.epa.gov/efservice/nn_ldc_nat_gas_deliveries/json") # - matches eia 176 total volumes.  not sales volume and no customer numbers.  Good way to auto-match?
...
#this dataset is in a somewhat difficult to use organization.  Subset to year of
#interest first
ghgrp_nn_ldc_emissions <- ghgrp_nn_ldc_emissions[ghgrp_nn_ldc_emissions$reporting_year==inventory_year,]
ghgrp_nn_ldc_emissions <- ghgrp_nn_ldc_emissions[,c("facility_id","volume_delivered","facility_name","end_user_category")]

#reorganize dataframe for ease of use
ghgrp_nn_ldc_emissions <- (reshape(ghgrp_nn_ldc_emissions, direction = "wide", idvar = c("facility_id","facility_name"), timevar = "end_user_category"))
rownames(ghgrp_nn_ldc_emissions) <- NULL

ghgrp_ldc_emissions <- merge(ghgrp_w_only_emissions,ghgrp_nn_ldc_emissions,by=c("facility_id","facility_name"))

rm(ghgrp_nn_ldc_emissions,ghgrp_w_only_emissions)
################################################################################
...
ghgrp <- merge(ghgrp_facility_info,ghgrp_ldc_emissions,
               by="facility_id")

#convert the relevant columns to numeric class
ghgrp[,c("latitude","longitude","Reported_CH4",
         ...
         "volume_delivered.Industrial consumers")] <- apply(ghgrp[,c("latitude","longitude","Reported_CH4",
                                                                   "volume_delivered.Electricity generating facilities",
                                                                   "volume_delivered.Residential consumers",
                                                                   "volume_delivered.Commercial consumers",
...
rm(A,ghgrp_facility_info,ghgrp_ldc_emissions)
################################################################################

newcolumns <- c("ghgrp_ID","ghgrp_facility_name","ghgrp_reported_ch4")
EIA_csv[,newcolumns] <- NA

EIA_csv[,c(23:54)][is.na(EIA_csv[,c(23:54)])] <- 0

EIA_csv <- EIA_csv[substring(EIA_csv$Company,9,10) %in% state_name_list,]

for(A in 1:nrow(ghgrp)){
  match_indx <- which.min(abs(EIA_csv$`Residential Total Volume (Mcf)` - ghgrp$`volume_delivered.Residential consumers`[A]) + 
                            abs(EIA_csv$`Commercial Total Volume (Mcf)` - ghgrp$`volume_delivered.Commercial consumers`[A]) + 
                            abs(EIA_csv$`Industrial Total Volume (Mcf)` - ghgrp$`volume_delivered.Industrial consumers`[A]) + 
                            abs(EIA_csv$`Electric Total Volume (Mcf)` - ghgrp$`volume_delivered.Electricity generating facilities`[A]))
  
  EIA_csv[match_indx,newcolumns] <- ghgrp[A,c("facility_id","facility_name.x","Reported_CH4")]
}
...
shp_domain_sf_LCC <- st_union(all_merge_state_sf_LCC[which(all_merge_state_sf_LCC$State %in% state_name_list), 'geometry'])
