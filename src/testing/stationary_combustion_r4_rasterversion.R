...
Stationary_combustion <- function(){
  ...
  if(Use_ACES){
    ...
    all_merge_LCC_state <- as(all_merge_LCC_state,"Spatial")
    all_merge_LCC_domain <- as(all_merge_LCC_domain,"Spatial")
    aces_res <- raster(aces_res)
    aces_com <- raster(aces_com)
    aces_ind <- raster(aces_ind)
    aces_elec <- raster(aces_elec)
    ...
    #of spatvectors
    cover_all <- raster::extract(aces_res,all_merge_LCC_state,weights=T,
    ...
    #   as("Spatial") %>%
    #   lapply(function(x){raster::cellFromPolygon(raster(aces_res),x,weights=T)*100})
  ...
  if(Use_Vulcan){
    ...
    all_merge_LCC_state <- as(all_merge_LCC_state,"Spatial")
    all_merge_LCC_domain <- as(all_merge_LCC_domain,"Spatial")
    vu_res <- raster(vu_res)
    vu_com <- raster(vu_com)
    vu_ind <- raster(vu_ind)
    vu_elec <- raster(vu_elec)
    cover_all <- raster::extract(vu_res,all_merge_LCC_state,weights=T,
                                 cellnumbers=T,normalizeWeights=F,exact=T)
    # cover_all <- all_merge_LCC_state %>% 
    #   split(f=paste0(all_merge_LCC_state$STATEFP,all_merge_LCC_state$COUNTYFP)) %>%
  ...
  if(XESMF){
    ...
    save_data <- function(input){
      ...
      disaggregation_level <- substring(text = input_name,regexpr("by",input_name),
                                        regexpr("\\[",input_name)-1)
      inventory_name <- strsplit(input_name,"_")[[1]][1]
      input <- project(rast(input),domain)
      #project to a grid with the exact right resolution, extent and origin.
      writeCDF(input,
               paste0(output_directory,'/testing/',inventory_name,'_',disaggregation_level,'_stat_comb_',total,'_regridded.nc'),
               force_v4=TRUE,
               varname='methane_emissions',
               unit='mol/km2/s',
               longname=paste0(inventory_name,'_',disaggregation_level,'_stat_comb_',total),
