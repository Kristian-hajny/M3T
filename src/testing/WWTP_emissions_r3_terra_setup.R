...
Output_directory <- "C:/Users/Kristian/Desktop"
...
central_rast <- rasterize(Municipal_flow_crop_filt, rast(d03_rast), "emiss", fun=sum)
...
for(A in 1:length(open_rasterfile)){
  ...
  states_trans <- project(states,crs(nlcd_frac))
  
  state_poly <- subset(states_trans, states_trans$STUSPS==State_info[A,1])
...
# Now just the names, coordinates and emissions
ghgrp_crop_output <- data.frame(ghgrp_crop$`FACILITY NAME`,
...
################################################################################
# Write the rasters
writeRaster(raster(central_flux),
...
writeRaster(raster(septic_flux),
...
writeRaster(raster(septic_flux2),
